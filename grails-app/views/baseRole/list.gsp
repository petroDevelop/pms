	...
	<head>
		...
		<script>
			function deleteFormatter(value, row) {
				var str='<a href="${request.contextPath}/baseRole/delete/'+row.id+'"><button class="btn btn-default margin" type="button"><span class="glyphicon glyphicon-trash"></span> &nbsp;<g:message code="default.button.delete.label" default="Delete" /></button></a>';
				return str;
			}
			function editFormatter(value, row) {
				var str='<a href="${request.contextPath}/baseRole/edit/'+row.id+'"><button class="btn btn-default margin" type="button"><span class="glyphicon glyphicon-edit"></span> &nbsp;<g:message code="default.button.edit.label" default="Edit" /></button></a>';
				return str;
			}
			function nameFormatter(value, row) {
				var str='<a href="${request.contextPath}/baseRole/show/'+row.id+'">'+row.name+'</a>';
				return str;
			}
			...
			function deleteAll(){
				var selects=$('#baseRoleTable').bootstrapTable('getSelections');
				if(selects.length>0){
					var ids=new Array();
			...
			function importExcel(){
				$('#excelForm').form('submit', {
					url:'${request.contextPath}/baseRole/importExel',
			...
			function newWindow(){
				document.location.href="${createLink(controller: 'baseRole',action:'create')}";
			}
		</script>
	</head>
	<body>
	...
	<div >
		...
		<div class="row">
			<ol class="breadcrumb">
				...
				<li class="active" ><g:message code="baseRole.label" default="BaseRole" /></li>
			</ol>
		</div><!--/.row-->

		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header"><g:message code="baseRole.label" default="BaseRole" /></h1>
			</div>
		</div><!--/.row-->
				...
				<div class="panel panel-default">
					<g:form method="post">
						...
						<div class="panel-body">
							...
							<table id="baseRoleTable" data-toggle="table" data-url="${request.contextPath}/baseRole/json"
								...
								<tr>
									...
									<th data-field="authority"  data-sortable="true"   >${message(code: 'baseRole.authority.label', default: 'Authority')}</th>
									
									<th data-field="name"  data-sortable="true"  data-formatter="nameFormatter" >${message(code: 'baseRole.name.label', default: 'Name')}</th>
									
									<th data-field="type"  data-sortable="true"   >${message(code: 'baseRole.type.label', default: 'Type')}</th>
