...
<html>
	<head>
		...
		<script>
			...
			function deleteAll(){
				var selects=$('#equipmentRunningInfoTable').bootstrapTable('getSelections');
				if(selects.length>0){
					var ids=new Array();
					...
					if(confirm('${message(code: 'default.button.delete.confirm.message', default: 'Are you sure?')}')) {
						$.post("${request.contextPath}/equipmentRunningInfo/deleteAll", obj,
			...
			function formSubmitAction(){
				$('#equipmentRunningInfoForm').form('validate');
				//$('#equipmentRunningInfoForm').validator('validate');
				$('#equipmentRunningInfoForm').form('submit', {
					url:'${request.contextPath}/equipmentRunningInfo/serverSave',
			...
			function importExcel(){
				$('#excelForm').form('submit', {
					url:'${request.contextPath}/equipmentRunningInfo/importExel',
					success: function(data){
						var data = eval('(' + data + ')'); // change the JSON string to javascript object
						$('#myModal').modal('hide');
						if (data.result){
	...
	<div >
		...
		<div class="row">
			<ol class="breadcrumb">
				...
				<li class="active" ><g:message code="equipmentRunningInfo.label" default="EquipmentRunningInfo" /></li>
			</ol>
		</div><!--/.row-->
				...
				<div class="panel panel-default">
					<g:form method="post">
						...
						<div class="panel-body">
							...
							<table id="equipmentRunningInfoTable" data-toggle="table" data-url="${request.contextPath}/equipmentRunningInfo/json"
								...
								<tr>
									...
									<th data-field="lastOverhaulTime"  data-sortable="true"   >${message(code: 'equipmentRunningInfo.lastOverhaulTime.label', default: 'Last Overhaul Time')}</th>
									
									<th data-field="maintenanceInitTime"  data-sortable="true"   >${message(code: 'equipmentRunningInfo.maintenanceInitTime.label', default: 'Maintenance Init Time')}</th>
									
									<th data-field="runningTime"  data-sortable="true"   >${message(code: 'equipmentRunningInfo.runningTime.label', default: 'Running Time')}</th>
									
									<th data-field="totalTime"  data-sortable="true"   >${message(code: 'equipmentRunningInfo.totalTime.label', default: 'Total Time')}</th>
		...
		<div class="row box animated tile"  id="box-edit">
			<div class="col-lg-12 form-group">
				<form  role="form" id="equipmentRunningInfoForm" enctype="multipart/form-data" method="post" >
	...
	<script>
		$("#equipmentRunningInfoForm").submit(function(e){
			e.preventDefault();
			formSubmitAction();
		});
	</script>
